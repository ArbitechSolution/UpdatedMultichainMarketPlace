...
const NftDetails = () => {
	...
	const buyFixedNft = async (collectionid, tokenid, amount) => {
		if (web3Api) {
			...
			let amountIn = web3Api.utils.toBN(
				fromExponential(parseFloat(amount) * Math.pow(10, 18))
			);

			nftContract.methods
				.buynft(collectionid, tokenid)
				.send({
					from: currentAccount,
					value: amountIn,
			...
			nftContract.methods
				...
				.then((value) => {
					var aucde = {
						id: value[1],
						val:
							Number(value[0])?.length > 21
								? Number(value[0]) / 1000000000000000000000000000000000000
								: Number(value[0]) / 1000000000000000000,
						userid: id,
					};
					setAucOwner(aucde);
				})
	...
	const removeAuction = async (tokenid) => {
		if (web3Api) {
			...
			nftContract.methods
				.removesfromauction(tokenid)
				.send({ from: currentAccount, gasPrice: gasPriceNumber })
				.then((result) => {
					console.log(result);
					setShow(false);
	...
	const buyAuctionNft = async (tokenid) => {
		if (
			(Number(aucBuyPrice) * 1000000000000000000 * 1.02) / 1000000000000000000 >
			Number(mybalance) + 0.003
		)
			return false;
		...
		if (web3Api) {
			...
			const gasPriceNumber = await getGasPrice();
			let amountIn = web3Api.utils.toBN(
				fromExponential(parseFloat(aucBuyPrice) * Math.pow(10, 18))
			);
	...
	const fixedSale = async (tokenid, price) => {
		...
		if (web3Api) {
			const gasPriceNumber = await getGasPrice();
			let priceIn = web3Api.utils.toBN(
				fromExponential(parseFloat(price) * Math.pow(10, 18))
			);

			nftContract.methods
				.fixedsales(tokenid, priceIn)
				.send({ from: currentAccount, gasPrice: gasPriceNumber })
				.then((result) => {
	...
	const auction = async (tokenid, price, endday, endhours) => {
		if (web3Api) {
			...
			nftContract.methods
				.startauction(tokenid, priceIn, endday, endhours)
				.send({ from: currentAccount, gasPrice: gasPriceNumber })
				.then((recipt) => {
	...
	const getEstimatedFee = async (tokenId, endpointChainId, chainId) => {
		const rpcUrl = CHAIN_INFO[chainId].rpcUrls; // Replace with your RPC URL
		console.log('rpcUrl', rpcUrl);
		const web3 = new Web3(rpcUrl[0]);

		const gasForDestinationLzReceive = 3500000;
		const collectionId = await nftContract.methods
			.nftcollectionid(tokenId)
			.call();
		const tokenURI = await nftContract.methods.tokenURI(tokenId).call();
		let nftInformation = await nftContract.methods
			.nftinformation(tokenId)
			.call();
		nftInformation = Object.values(nftInformation);

		if (chainId && web3Api && currentAccount) {
			const endpointContract = new web3.eth.Contract(
				contractAbI,
				EndpointAddress[endpointChainId]
			);
			const adapterParams = web3.eth.abi.encodeParameters(
				['uint16', 'uint256'],
				[1, gasForDestinationLzReceive]
			);
			...
			// Call estimateFees function
			endpointContract.methods
				.estimateFees(
					endpointChainId,
					EndpointAddress[endpointChainId],
					payload,
				...
				.call({ from: currentAccount }, (error, result) => {
					...
					const messageFee = result[0];
					console.log('Estimated message fee:', messageFee);
				});
		}
	};

	const sellOnBinanceChain = async (tokenid, chainId) => {
		setShow(true);
		setBtnName('traverse');
		if (web3Api) {
			const gasPriceNumber = await getGasPrice();
			const layerZeroChainId = CHAIN_INFO[chainId].endPointChainId;
			console.log('endpointChainId', layerZeroChainId);
			// await getEstimatedFee(tokenid, layerZeroChainId, chainId);
			let value = '50000000000000000';
			nftContract.methods
				.traverseChains(layerZeroChainId, tokenid)
				.estimateGas({
					from: currentAccount,
					value: value,
				})
	...
	const allChains = () => {
		const excludedChainIds = ['56', '324', '42161', '80001', chainId];
		const testnetData = Object.entries(CHAIN_INFO).filter(([key, value]) => {
			return !excludedChainIds.includes(key);
		});
		console.log('testnetData', testnetData);
		setChainsData(testnetData);
	};
	return (
				...
				<div className="autoContainer">
					<div className="productDetails__inner">
						<div className="productDetails__inner-image">
							{nftData &&
								(VIDEO_TYPE.includes(JSON.parse(nftData[5])[1]) ? (
									<video alt="" muted autoPlay loop>
										<source
											src={getCanonicalPath(IPFS_BASE_URL + nftData[6])}
										/>
									</video>
						...
						<div className="productDetails__inner-info">
							...
							<div className="productDetails__inner-info-text">
								<h5>{aucBuyPrice > 0 ? 'Auction Min Price' : 'List Price'}</h5>
								<p>
									<span>
										<img src="/assets/images/zk/eth.svg" alt="" />
									</span>
									{listPrice}{' '}
									{chainId && CHAIN_INFO[chainId].nativeCurrency.symbol}
								</p>
							</div>
							<div className="productDetails__inner-info-text">
								<h5>Taker Fee</h5>
								<p>
									...
									{(Number(listPrice) * 1000000000000000000 * 0.015) /
										1000000000000000000}{' '}
									{chainId && CHAIN_INFO[chainId].nativeCurrency.symbol}
								</p>
							</div>
							<div className="productDetails__inner-info-text">
								...
								<p>
									...
									{(Number(listPrice) * 1000000000000000000 * 0.05) /
										1000000000000000000}{' '}
									{chainId && CHAIN_INFO[chainId].nativeCurrency.symbol}
								</p>
							</div>
							<div className="productDetails__inner-info-total">
								...
								<p>
									...
									{(Number(listPrice) * 1000000000000000000 * 1.02) /
										1000000000000000000}{' '}
									{chainId && CHAIN_INFO[chainId].nativeCurrency.symbol}
								</p>
							</div>
							<div className="productDetails__inner-info-pay">
								...
								<p>Connect supported wallet • Fastest</p>
								<div className="arrow">
									<img src="/assets/images/zk/arrow-down.svg" alt="" />
								</div>
							</div>
							{(aucBuyPrice > 0
								? (Number(aucBuyPrice) * 1000000000000000000 * 1.02) /
								  1000000000000000000
								: (Number(listPrice) * 1000000000000000000 * 1.02) /
								  1000000000000000000) >
								Number(mybalance) + 0.003 && (
							...
							{buyPrice > 0 &&
							nOwner?.toLowerCase() ===
								currentAccount?.toLowerCase() ? null : buyPrice > 0 ? (
								<div className="productDetails__inner-info-text">
									<button
										className="button--primary"
										style={{ width: '100%', borderRadius: 8 }}
										onClick={() =>
											buyFixedNft(nftData[7], nftData[0], buyPrice)
										}
									>
										Buy now{' '}
										{btnName == 'buynft' && show && (
											<i className="fas fa-spinner fa-pulse fa-1x ml-3"></i>
										)}
									</button>
								</div>
							) : null}
							...
							aucTime?.h === 0 &&
							aucTime?.m === 0 &&
							aucOwner?.id.toLowerCase() === currentAccount?.toLowerCase() ? (
								aucBuyPrice > 0 && aucOwner?.val >= aucBuyPrice ? (
									<div className="productDetails__inner-info-text">
										<button
											style={{ width: '100%', borderRadius: 8 }}
											className="button--primary"
											onClick={() => claimAuctionNft(nftData[7], nftData[0])}
										>
											CLAIM{' '}
											{btnName == 'claimauc' && show && (
												<i className="fas fa-spinner fa-pulse fa-1x ml-3"></i>
											)}
										</button>
									</div>
								) : Number(aucBuyPrice) > 0 &&
								  Number(aucBuyPrice) > aucOwner?.val &&
								  nOwner?.toLowerCase() === currentAccount?.toLowerCase() ? (
									<div className="productDetails__inner-info-text">
										<button
											style={{ width: '100%', borderRadius: 8 }}
											className="button--primary"
											onClick={() => removeAuction(nftData[0])}
										>
											Cancel Auction{' '}
											{btnName == 'cancelauc' && show && (
												<i className="fas fa-spinner fa-pulse fa-1x ml-3"></i>
											)}
										</button>
									</div>
							...
							{nOwner?.toLowerCase() ===
							currentAccount?.toLowerCase() ? null : openBid ? null : aucBuyPrice >
							  0 ? (
								aucTime?.d === 0 &&
								aucTime?.h === 0 &&
								aucTime?.m === 0 ? null : (
								...
								{openBid ? (
									<div className="productDetails__inner-info-text">
										<input
											...
											step="any"
											min={
												aucBuyPrice > aucOwner?.val
													? aucBuyPrice
													: aucOwner?.val
											}
											onBlur={(e) => setAucBuyPrice(Number(e.target.value))}
											required
										/>
									</div>
								) : null}
								...
								chainId === '421613' ? (
									<div className="productDetails__inner-info-text">
										<button
											style={{ width: '100%', borderRadius: 8 }}
											className="button--primary"
											onClick={() => sellOnBinanceChain(nftid)}
										>
											Sell on Binance Chain{' '}
											{btnName == 'traverse' && show && (
												<i className="fas fa-spinner fa-pulse fa-1x ml-3"></i>
											)}
										</button>
									</div>
							...
							<div className="dropdown">
								<button className="dropbtn">
									{' '}
									{chainsData.length > 0
										? 'Sell On ' + chainsData[0][1].chainName
										: 'Loading...'}
								</button>
									...
									{chainsData.length > 0 ? (
										chainsData.map((chain, index) => (
											<a
												href="#"
												key={index}
												onClick={() => sellOnBinanceChain(nftid, chain[0])}
											>
												{chain[1].chainName}
											</a>
										))
							...
							{nftData &&
							nftData[8].toLowerCase() === currentAccount?.toLowerCase() ? (
								<form
									onSubmit={(e) => {
										e.preventDefault();
								...
								>
									...
									<div className="productDetails__inner-info-text">
										<input
											type="Number"
											placeholder="Enter Min Bid Value in ETH"
											step="any"
											min={
												aucBuyPrice > aucOwner?.val
													? aucBuyPrice
							...
							nOwner?.toLowerCase() === currentAccount?.toLowerCase() ? (
								<div className="productDetails__inner-info-text">
									<button
										className="button--primary"
										style={{ width: '100%', borderRadius: 8 }}
										onClick={() => removeSale(nftData[7], nftData[0])}
									>
										Cancel Sale{' '}
										{btnName == 'cancelsale' && show && (
											<i className="fas fa-spinner fa-pulse fa-1x ml-3"></i>
										)}
									</button>
								</div>
